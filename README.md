...
5개 테이블을 조인하면 주문 1건이 여러 행으로 펼쳐집니다.

- 이전 버전: `GROUP_CONCAT`으로 토핑을 한 문자열로 합치고, 화면에서 이전 행의 주문번호를 기억해 중복을 건너뛰는 방식 → 화면 기술(JSP → Thymeleaf)을 바꾸자 동작하지 않음
- 현재: MyBatis `<collection>`으로 주문 → 아이스크림 → 토핑 중첩 객체로 바로 매핑
